function Vnm = tess2Vnm_fft(lon, lat, r1, r2, rho, nmax, opt)
%{
    This function calculates the potential coefficients generated by
    tesseroids with constant density using FFT-based algorithm
Input:
...
if isempty(opt.szm)
    ...
    dlam = deg2rad(abs(lon(2) - lon(1)));
else
    dlat = opt.szm(1);
    dlam = deg2rad(opt.szm(2));
end
...
Vnm = Qnm;
t1 = nlam * (exp(1i * m * dlam) - 1) ./ (1i * m) .* exp(1i * m * -pi);
t1 = t1.';
